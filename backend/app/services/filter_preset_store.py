...
from app.models.workspace_prefs import FilterPreset, FilterPresetCreate

_DATA_DIR = Path(__file__).resolve().parents[2] / "data" / "filter_presets"
_CACHE: dict[tuple[int, str | None], tuple[float, list[FilterPreset]]] = {}
_CACHE_TTL = 60
...
def list_filter_presets(survey_id: int, username: str | None = None) -> list[FilterPreset]:
    ...
    presets = [FilterPreset.model_validate(row) for row in rows]
    _CACHE[cache_key] = (now, presets)
    return presets
